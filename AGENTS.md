...
| `src/UsageBar/Domain/UsageModels.cs` | Provider interface (`IUsageProvider`), usage model records (`ProviderResult`, `UsageBarWindow`, `UsageBlock`, `ProviderCredentials`). `UsageBarWindow` carries provider name, window label, and used percent for icon rendering and limit-reset notifications. |
...
| `src/UsageBar/Providers/` | Codex, Claude, DeepSeek, OpenRouter, Deepgram provider implementations and JSON helpers. |
...
- Manual validation should cover:
  ...
  - Tray icon dynamically shows bars for each provider's usage windows with correct separator widths and color gradient.
...
- `NotifyLimitRefreshes` compares each provider's usage windows against the previous snapshot. When any window's used percent drops (indicating a limit reset), a Windows notification is shown with the provider and window label (e.g., "Claude 5h limit refreshed").
...
- Tray icon is generated by `IconFactory.CreateUsageIcon` from a list of `UsageBarWindow` values.
- `UsageBarWindow` carries a provider name, window label (e.g. "5h", "7d"), and a used percentage.
- Icon layout is dynamic: bars are stacked vertically with separators (1 px within the same provider, 2 px between different providers). Layout cases handled:
  - Single provider (Codex or Claude only, one or two windows) → proportional stack
  - Both Codex and Claude present → 25-25-25-25 or 50-25-25 split depending on available windows
  - No windows → empty gray bar
- Bar fill color follows a continuous gradient: 0% green, 50% yellow, 100% red. Gray is used when no usage data is available.
...
- Return display-ready `UsageBlock` values and `UsageBarWindow` values for icon rendering.
