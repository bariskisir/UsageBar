...
internal sealed class UsageHttpTelemetryHandler(ILogger<UsageHttpTelemetryHandler> logger) : DelegatingHandler
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        logger.LogInformation(
            "HTTP request started: {Method} {Uri}; headers={Headers}; contentType={ContentType}; contentLength={ContentLength}.",
            request.Method.Method,
            safeUri,
            SensitiveDataRedactor.HeaderNames(request.Headers, request.Content?.Headers),
        ...
        try
        {
            ...
            logger.LogInformation(
                "HTTP response received: status={StatusCode}; success={Success}; durationMs={DurationMs:F1}; headers={Headers}; contentType={ContentType}; contentLength={ContentLength}.",
                (int)response.StatusCode,
                response.IsSuccessStatusCode,
                elapsed.TotalMilliseconds,
                SensitiveDataRedactor.HeaderNames(response.Headers, response.Content.Headers),
        ...
        {
            logger.LogInformation(
                "HTTP request cancelled by caller after {DurationMs:F1} ms.",
        ...
        {
            logger.LogWarning(
                "HTTP request timed out after {DurationMs:F1} ms.",
                Stopwatch.GetElapsedTime(started).TotalMilliseconds);
            throw;
        }
        ...
        {
            logger.LogWarning(
                "HTTP transport failure after {DurationMs:F1} ms: type={ExceptionType}; status={StatusCode}; hresult={HResult}.",
                Stopwatch.GetElapsedTime(started).TotalMilliseconds,
    ...
    private async Task LogBodyAsync(string direction, HttpContent? content, CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogDebug(
                "HTTP {Direction} body omitted; enable USAGEBAR_HTTP_BODY_LOGGING for a redacted diagnostic snapshot.",
                direction);
            return;
        }
        ...
        try
        {
            ...
            logger.LogDebug(
                "HTTP {Direction} body: fingerprint={Fingerprint}; snapshot={BodySnapshot}",
                direction,
                SensitiveDataRedactor.BodyFingerprint(body),
                SensitiveDataRedactor.BodySnapshot(body, content.Headers.ContentType?.MediaType));
        }
