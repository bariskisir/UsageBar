...
public sealed class WarpProvider(HttpClient httpClient) : IUsageProvider
{
    ...
    public async Task<ProviderResult?> GetUsageAsync(ProviderQueryContext context, CancellationToken cancellationToken)
    {
        ...
        if (ProviderJson.TryGetProperty(root, "errors", out var errors) &&
        ...
        var info = root;
        foreach (var key in new[] { "data", "user", "user", "requestLimitInfo" })
        {
        ...
        if (nextRefresh is not null &&
            DateTimeOffset.TryParse(nextRefresh, null, System.Globalization.DateTimeStyles.AssumeUniversal, out var parsed))
        {
    ...
    {
        var userObj = root;
        foreach (var key in new[] { "data", "user", "user" })
        {
        ...
        if (ProviderJson.TryGetProperty(userObj, "bonusGrants", out var bonusGrants) &&
            bonusGrants.ValueKind == JsonValueKind.Array)
        {
        ...
        if (ProviderJson.TryGetProperty(userObj, "workspaces", out var workspaces) &&
            workspaces.ValueKind == JsonValueKind.Array)
        {
            foreach (var workspace in workspaces.EnumerateArray())
            {
                if (ProviderJson.TryGetProperty(workspace, "bonusGrantsInfo", out var bonusGrantsInfo) &&
                    ProviderJson.TryGetProperty(bonusGrantsInfo, "grants", out var wsGrants) &&
                    wsGrants.ValueKind == JsonValueKind.Array)
                {
    ...
    private static double AsDouble(JsonElement element, string propertyName)
    {
        ...
        if (prop.ValueKind == JsonValueKind.String &&
            double.TryParse(prop.GetString(), System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out var s))
        {
            return s;
        }
