...
public sealed class AlibabaProvider(HttpClient httpClient) : ISingleResultUsageProvider
{
    ...
    public ProviderDescriptor Descriptor { get; } = new("Alibaba", 23, ProviderAuthenticationKind.ApiKey, CredentialNames.Alibaba, 21, "alibaba", ["alibaba_5h", "alibaba_weekly", "alibaba_monthly"]);

    public bool IsConfigured(ProviderQueryContext context) => !string.IsNullOrEmpty(context.GetApiKey(CredentialNames.Alibaba));
    public async Task<ProviderResult?> GetUsageAsync(ProviderQueryContext context, CancellationToken cancellationToken)
    {
        ...
        )
        {
            ...
            using (var document = await ProviderHttp.GetJsonAsync(httpClient, request, cancellationToken).ConfigureAwait(false))
            {
                ...
                {
                    if (ProviderJson.TryGetProperty(data, key, out var next) && next.ValueKind == System.Text.Json.JsonValueKind.Object)
                    {
                        data = next;
                    }
                }
    ...
    private static void TryAddWindow(System.Text.Json.JsonElement element, string key, string label, List<UsageWindow> windows, DateTimeOffset now)
    {
        var percent = ProviderJson.GetDouble(element, $"{key}UsedPercent") ?? ProviderJson.GetDouble(element, $"usedPercent{key}") ?? ProviderJson.GetDouble(element, key);
    ...
    private static UsageWindow? ReadQuotaWindow(System.Text.Json.JsonElement quota, string providerName, DateTimeOffset now)
    {
        ...
        var usedPercent = ProviderJson.GetDouble(quota, "usedPercent") ?? ProviderJson.GetDouble(quota, "used_percent") ?? ProviderJson.GetDouble(quota, "usagePercent");
        ...
        if (resetTime is not null && DateTimeOffset.TryParse(resetTime, null, System.Globalization.DateTimeStyles.AssumeUniversal, out var parsed))
        {
