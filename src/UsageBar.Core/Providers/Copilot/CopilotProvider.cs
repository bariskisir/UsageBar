...
public sealed class CopilotProvider(HttpClient httpClient) : ISingleResultUsageProvider
{
    private const string CopilotEndpoint = "https://api.github.com/copilot_internal/user";
    public ProviderDescriptor Descriptor { get; } = new("Copilot", 5, ProviderAuthenticationKind.ApiKey, CredentialNames.Copilot, 12, "copilot", ["copilot_premium", "copilot_chat"]);

    public bool IsConfigured(ProviderQueryContext context) => !string.IsNullOrEmpty(context.GetApiKey(CredentialNames.Copilot));
    public async Task<ProviderResult?> GetUsageAsync(ProviderQueryContext context, CancellationToken cancellationToken)
    {
        ...
        using (var request = new HttpRequestMessage(HttpMethod.Get, CopilotEndpoint))
        {
            ...
            using (var document = await ProviderHttp.GetJsonAsync(httpClient, request, cancellationToken).ConfigureAwait(false))
            {
                ...
                if (resetDate is not null && DateTimeOffset.TryParse(resetDate, null, System.Globalization.DateTimeStyles.AssumeUniversal, out var pr))
                {
                    parsedReset = pr;
                }
    ...
    private static UsageWindow? ReadQuotaWindow(System.Text.Json.JsonElement snapshots, string snapshotKey, string label, string providerName, DateTimeOffset now, DateTimeOffset? resetDate)
    {
        ...
        var isUnlimited = ProviderJson.TryGetProperty(quota, "unlimited", out var ul) && ul.ValueKind != System.Text.Json.JsonValueKind.False;
        double usedPercent;
        if (isUnlimited)
        {
            usedPercent = 0;
