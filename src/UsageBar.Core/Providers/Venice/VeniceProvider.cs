...
{
    public override ProviderDescriptor Descriptor { get; } = new("Venice", 108, ProviderAuthenticationKind.ApiKey, CredentialNames.Venice, SettingsOrder: 11);
    ...
    protected override async Task<BalanceFetchResult> FetchBalanceAsync(HttpClient httpClient, string apiKey, CancellationToken cancellationToken)
    {
        using (var document = await ProviderHttp.GetJsonWithBearerAsync(httpClient, "https://api.venice.ai/api/v1/billing/balance", apiKey, cancellationToken).ConfigureAwait(false))
        {
            ...
            var canConsume = ProviderJson.TryGetProperty(root, "canConsume", out var cc) && cc.ValueKind == System.Text.Json.JsonValueKind.True;
            ...
            var usd = balances.ValueKind != System.Text.Json.JsonValueKind.Undefined ? ProviderJson.GetDecimal(balances, "usd") : ProviderJson.GetDecimal(root, "usd");
            var diem = balances.ValueKind != System.Text.Json.JsonValueKind.Undefined ? ProviderJson.GetDecimal(balances, "diem") : ProviderJson.GetDecimal(root, "diem");
