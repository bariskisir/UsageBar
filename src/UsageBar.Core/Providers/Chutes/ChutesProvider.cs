...
public sealed class ChutesProvider(HttpClient httpClient) : IUsageProvider
{
    ...
    public async Task<ProviderResult?> GetUsageAsync(ProviderQueryContext context, CancellationToken cancellationToken)
    {
        ...
        var fourHourPercent = ProviderJson.GetDouble(root, "usedPercent") ??
            ProviderJson.GetDouble(root, "fourHourUsedPercent") ??
            ProviderJson.GetDouble(root, "rolling_4h_used");
        var monthlyPercent = ProviderJson.GetDouble(root, "monthlyUsedPercent") ??
            ProviderJson.GetDouble(root, "monthly_used");
