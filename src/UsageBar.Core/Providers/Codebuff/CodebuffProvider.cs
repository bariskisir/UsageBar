...
public sealed class CodebuffProvider(HttpClient httpClient) : ISingleResultUsageProvider
{
    private const string DefaultBaseUrl = "https://www.codebuff.com";
    public ProviderDescriptor Descriptor { get; } = new("Codebuff", 27, ProviderAuthenticationKind.ApiKey, CredentialNames.Codebuff, 14, "codebuff", ["codebuff_quota"]);

    public bool IsConfigured(ProviderQueryContext context) => !string.IsNullOrEmpty(context.GetApiKey(CredentialNames.Codebuff));
    public async Task<ProviderResult?> GetUsageAsync(ProviderQueryContext context, CancellationToken cancellationToken)
    {
        ...
        )
        {
            ...
            using (var document = await ProviderHttp.GetJsonAsync(httpClient, request, cancellationToken).ConfigureAwait(false))
            {
                ...
                if (resetDate is not null && DateTimeOffset.TryParse(resetDate, null, System.Globalization.DateTimeStyles.AssumeUniversal, out var parsed))
                {
