...
public sealed class ProviderHttpTests
{
    ...
    public async Task GetJsonAsync_throws_on_non_success_status()
    {
        using (var httpClient = new HttpClient(FakeHttpMessageHandler.Json("{}", HttpStatusCode.InternalServerError)))
        {
            using (var request = new HttpRequestMessage(HttpMethod.Get, "https://test.example/api"))
            {
                await Assert.ThrowsAsync<HttpRequestException>(() => ProviderHttp.GetJsonAsync(httpClient, request, CancellationToken.None));
            }
        }
    }
    ...
    public async Task GetJsonAsync_throws_on_invalid_json()
    {
        ...
        var handler = new FakeHttpMessageHandler(_ => new HttpResponseMessage(HttpStatusCode.OK) { Content = new StringContent("not-json", System.Text.Encoding.UTF8, "application/json"), });
        using (var httpClient = new HttpClient(handler))
        {
            using (var request = new HttpRequestMessage(HttpMethod.Get, "https://test.example/api"))
            {
                // Invalid JSON throws a System.Text.Json parse exception.
                var ex = await Record.ExceptionAsync(() => ProviderHttp.GetJsonAsync(httpClient, request, CancellationToken.None));
                Assert.NotNull(ex);
    ...
    public async Task GetJsonWithBearerAsync_throws_on_non_success_status()
    {
        ...
        using (var httpClient = new HttpClient(handler))
        {
            await Assert.ThrowsAsync<HttpRequestException>(() => ProviderHttp.GetJsonWithBearerAsync(httpClient, "https://test.example/api/v2", "invalid-key", CancellationToken.None));
        }
    }
}