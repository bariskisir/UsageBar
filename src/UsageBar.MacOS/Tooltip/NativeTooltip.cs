...
internal sealed class NativeTooltip : INativeTooltip, IDisposable
{
    ...
    public NativeTooltip()
    {
        ...
        config.UserContentController.AddScriptMessageHandler(
            new ScriptMessageHandler(OnScriptMessage), "ipc");
    ...
    {
        var json = JsonSerializer.Serialize(new { cards }, TooltipJsonContext.Default.TooltipPayload);
        lock (_gate)
        {
            _pendingJson = json;
            if (_ready)
    ...
    private static string LoadHtml()
    {
        ...
        return html
            .Replace("{{CODEX_ICON}}", ReadSvgDataUri(assembly, "UsageBar.Core.Providers.Codex.codex.svg"), StringComparison.Ordinal)
            ...
            .Replace("{{DEEPSEEK_ICON}}", ReadSvgDataUri(assembly, "UsageBar.Core.Providers.DeepSeek.deepseek.svg"), StringComparison.Ordinal)
            .Replace("{{OPENROUTER_ICON}}", ReadSvgDataUri(assembly, "UsageBar.Core.Providers.OpenRouter.openrouter.svg"), StringComparison.Ordinal)
            .Replace("{{MOONSHOT_ICON}}", ReadSvgDataUri(assembly, "UsageBar.Core.Providers.Moonshot.moonshot.svg"), StringComparison.Ordinal)
            .Replace("{{DEEPGRAM_ICON}}", ReadSvgDataUri(assembly, "UsageBar.Core.Providers.Deepgram.deepgram.svg"), StringComparison.Ordinal)
            .Replace("{{OPENAI_ICON}}", ReadSvgDataUri(assembly, "UsageBar.Core.Providers.OpenAI.openai.svg"), StringComparison.Ordinal)
            .Replace("{{VENICE_ICON}}", ReadSvgDataUri(assembly, "UsageBar.Core.Providers.Venice.venice.svg"), StringComparison.Ordinal)
            .Replace("{{COPILOT_ICON}}", ReadSvgDataUri(assembly, "UsageBar.Core.Providers.Copilot.copilot.svg"), StringComparison.Ordinal)
            .Replace("{{CROF_ICON}}", ReadSvgDataUri(assembly, "UsageBar.Core.Providers.Crof.crof.svg"), StringComparison.Ordinal)
            ...
            .Replace("{{POE_ICON}}", ReadSvgDataUri(assembly, "UsageBar.Core.Providers.Poe.poe.svg"), StringComparison.Ordinal)
            .Replace("{{ALIBABA_ICON}}", ReadSvgDataUri(assembly, "UsageBar.Core.Providers.Alibaba.alibaba.svg"), StringComparison.Ordinal)
            .Replace("{{ZENMUX_ICON}}", ReadSvgDataUri(assembly, "UsageBar.Core.Providers.ZenMux.zenmux.svg"), StringComparison.Ordinal)
