...
internal sealed class NativeTray : INativeTray
{
    ...
    {
        var bars = new IconLayout.Bar[]
        {
            new(UsedPercent: null, Weight: 1.0, Provider: "None")
        };
    ...
    private static NSImage CreateImageFromRgba(byte[] rgba, int width, int height)
    {
        ...
        using (var provider = new CGDataProvider(rgba, 0, rgba.Length))
        {
            ...
            {
                using (var cgImage = new CGImage(width, height, bitsPerComponent: 8, bitsPerPixel: 32, bytesPerRow: bytesPerRow, colorSpace, CGBitmapFlags.ByteOrder32Little | CGBitmapFlags.PremultipliedFirst, provider, decode: null, shouldInterpolate: false, intent: CGColorRenderingIntent.Default))
                {
